...
it with new values. See <<executable-graph-update, Executable Graph Update>>
...
There are multiple classes that can be used to register a kernel node argument
as a dynamic parameter. Every class provides an `update()` method which
can be used to update the kernel argument associated with that instance. All classes
also provide the {crs}[common reference semantics]. The choice of which class
to use, depends on the kernel argument types that need to be updated:
...
- <<dynamic-accessor-class, dynamic_accessor class>>: used to update `sycl::accessor`
arguments.

- <<dynamic-local-accessor-class, dynamic_local_accessor class>>: used to update
`sycl::local_accessor` arguments.

- <<dynamic-work-group-memory-class, dynamic_work_group_memory class>>: used to update
`work_group_memory` arguments (as defined in the
...
[source,c++]
----
dynamic_accessor(command_graph<graph_state::modifiable> graph,
                  buffer &initialValue);
----
|Constructs a dynamic accessor object that can be registered with command graph
nodes with an initial value.
...
|Updates parameters in all nodes registered with this dynamic accessor to
`newValue`. This new value will be reflected immediately in the modifiable graph
which contains the registered nodes. The new value will not be reflected in any
executable graphs created from that modifiable graph until
`command_graph::update()` is called passing the modified nodes, or a new
...
dynamic_local_accessor(command_graph<graph_state::modifiable> graph, range<Dimensions> allocationSize);
----
...
* `graph` - Graph containing the nodes which will have dynamic parameters.
* `allocationSize` - The size 

|
[source,c++]
----
void update(const buffer& newValue);
----
...
[source,c++]
----
dynamic_work_group_memory(command_graph<graph_state::modifiable> graph,
